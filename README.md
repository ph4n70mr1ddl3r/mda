...
- [`docs/HARDENING.md`](./docs/HARDENING.md) — Phase-11 production hardening pass: release-mode E2E as the `mda_app` role, the works-as-owner bug class it exposed (schema misplacement, missing grants, runtime DDL), and the regression suites that now guard it. Later passes: SSE tenant-channel scoping, SMTP egress scrubbing, the `reference`-field modeling gate, the retire-referenced-entity publish gate, and the non-superuser webhook-lookup fix (`20260137000001`).
