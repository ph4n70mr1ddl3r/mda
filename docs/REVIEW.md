...
> **Status:** All critical gaps (C1–C6) and underspecified items (U1–U4, U6–U8) are resolved in `PLAN.md` (now v0.4); the timeline re-estimate and MVP milestone are addressed in §9. Only **U5 (data i18n)** and **U9 (HA/replication)** remain — deliberately deferred to later phases. Subsequent review passes produced further refinements recorded as **ADRs 0011–0026** (publish execution & atomic cutover, value-constraint composition, sharing materialization, reporting query model, deletion/restore lifecycle, chained workflow transitions, rollup summaries — then surfaced capabilities, scheduled jobs, platform follow-ups, mass actions, API versioning, i18n, GraphQL hot-invalidation, team hierarchy, and criteria sharing rules + live role hierarchy). A later as-built accuracy pass reconciled PLAN/README with the code; the latest pass additionally **closed two documented-but-missing API surfaces by implementing them** — `GET /api/schema/:entity` (JSON-Schema discovery, FLS-projected) and `GET /api/studio/snapshots/:id/model` (the rollback input) — and added as-built notes for the job-framework divergence (ADR-0007/0019), the OAuth2 deferral, the global event-log `seq`, and the SSE relay's v1 AuthZ grain. This review is retained as the reasoning trail; see `adr/` for all recorded decisions.
